...
# Common lay-terms that users type but which are disease names, not symptom names.
# These are intercepted BEFORE symptom matching so e.g. 'cold' -> 'Common Cold'
# instead of wrongly matching the 'cold_hands_and_feets' symptom.
# NOTE: 'fever' is intentionally NOT in this dict – it falls through to symptom
# matching where it correctly resolves to high_fever / mild_fever symptoms.
DISEASE_KEYWORD_ALIASES = {
    'cold':         'Common Cold',
    ...
    'haemorrhoids': 'Dimorphic hemmorhoids(piles)',
...
def keyword_to_symptoms(keyword):
    """Return symptom keys that EXACTLY match (word-level) the keyword.

    Uses word-boundary matching so that 'cold' does NOT match 'cold hands and feets'.
    The keyword must appear as a complete word (or full token) within the symptom name.
    """
    import re
    kw = keyword.strip().lower()
    kw_nospace = kw.replace(' ', '_')
    # Build a regex that requires the keyword to appear as a whole word
    pattern = re.compile(r'(?<![a-z])' + re.escape(kw) + r'(?![a-z])')
    pattern_us = re.compile(r'(?<![a-z])' + re.escape(kw_nospace) + r'(?![a-z])')
    matched = []
    for sym_key in symptoms_list_processed.keys():
        sym_us = sym_key.replace(' ', '_')
        if pattern.search(sym_key) or pattern_us.search(sym_us):
...
if predict_btn:
    if user_input:
        ...
        # Track if we already showed a disease-name result (for multi-keyword input)
        disease_name_result_shown = False
        ...
        for kw in raw_keywords:
            ...
            # ── Tier 0: Known lay-term alias (e.g. 'cold' → 'Common Cold') ──────
            alias_disease = DISEASE_KEYWORD_ALIASES.get(kw_lower)
            if alias_disease is not None:
                info_lines.append(f"💡 **'{kw}'** recognised as disease name → **{alias_disease}**")
                with st.spinner("🧠 Fetching disease info..."):
                    dis_des_v, prec_v, meds_v, diet_v, wrkt_v = information(alias_disease)
                ...
                <div style='background:linear-gradient(135deg,#e3f2fd,#ede7f6); border:2px solid #1e88e5;
                    ...
                    <div style='font-size:34px; font-weight:800; color:#0d47a1; margin:6px 0;'>{alias_disease}</div>
                ...
                <div style='background:#fff8e1; border-left:5px solid #ffa000; border-radius:10px;
                    ...
                    <span style='color:#e65100; font-size:13px;'>💡 <b>Tip:</b> You entered a disease name.
                    For a symptom-based AI prediction, try entering your actual symptoms separated by commas,
                    e.g. <i>runny nose, sore throat, cough, headache</i>.</span>
                </div>
                """, unsafe_allow_html=True)
                render_result_cards(dis_des_v, prec_v, meds_v, diet_v, wrkt_v)
                st.stop()

            # ── Tier 1: Disease name substring match ─────────────────────────────
            ...
            # ── Tier 2: Exact word-boundary symptom match ────────────────────────
