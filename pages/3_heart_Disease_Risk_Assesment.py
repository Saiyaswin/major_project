...
# ── Header Banner ──────────────────────────────────────────────────────────────
banner_img, banner_txt = st.columns([1, 3])
with banner_img:
    st.image("utils/heart_disease.jpg", use_container_width=True)
with banner_txt:
    st.markdown("""
    <div style='padding:18px 0 0 10px;'>
        <h1 style='color:#e53935; margin-bottom:6px;'>❤️ AI-Powered Heart Disease Assessment App</h1>
        <p style='font-size:17px; color:#555;'>Unmatched Accuracy with Cutting-Edge Machine Learning Models</p>
        <div style='display:flex; gap:10px; flex-wrap:wrap; margin-top:12px;'>
            <span style='background:#fff0f0;color:#c62828;padding:5px 14px;border-radius:20px;font-size:13px;font-weight:600;border:1px solid #ffcdd2;'>🤖 LightGBM Model</span>
            <span style='background:#fff3e0;color:#e65100;padding:5px 14px;border-radius:20px;font-size:13px;font-weight:600;border:1px solid #ffe0b2;'>📊 SHAP Explainability</span>
            <span style='background:#e8f5e9;color:#2e7d32;padding:5px 14px;border-radius:20px;font-size:13px;font-weight:600;border:1px solid #c8e6c9;'>🏥 400K+ Patient Dataset</span>
        </div>
    </div>
...
with row1_1:
    st.markdown("<h4 style='color:#e53935;'>👤 Demographics</h4>", unsafe_allow_html=True)
...
with row3_1:
    st.markdown("<h4 style='color:#1565c0;'>🏥 Medical History</h4>", unsafe_allow_html=True)
...
asthma = row4_1.selectbox("🌬️ Have you ever been diagnosed with asthma?", ["never_asthma", "current_asthma", "former_asthma"], index=0)
...
bmi = row4_2.selectbox("⚖️ What is your body mass index (BMI)?", [
...
with row5_1:
    st.markdown("<h4 style='color:#2e7d32;'>🌿 Lifestyle</h4>", unsafe_allow_html=True)
...
exercise_status = row6_3.selectbox("🏋️ Have you exercised in the past 30 days?", ["yes", "no"], index=0)
...
if btn1:
    try:
        ...
        with row8_1:
            ...
            if risk > 70:
                r_emoji, r_color, r_bg, r_border, r_label = "🔴", "#b71c1c", "#ffebee", "#ef9a9a", "Very High Risk ⚠️"
            elif risk > 40:
                r_emoji, r_color, r_bg, r_border, r_label = "🟠", "#e65100", "#fff3e0", "#ffcc80", "High Risk ⚠️"
            elif risk > 25:
                r_emoji, r_color, r_bg, r_border, r_label = "🟡", "#f57f17", "#fffde7", "#fff176", "Moderate Risk"
            else:
                r_emoji, r_color, r_bg, r_border, r_label = "🟢", "#2e7d32", "#e8f5e9", "#a5d6a7", "Low Risk ✅"

            st.markdown(f"""
            <div style="
                background: {r_bg};
                border: 2px solid {r_border};
                border-radius: 18px;
                padding: 28px 30px;
                margin: 16px 0;
                text-align: center;
            ...
            ">
                <div style="font-size:52px; margin-bottom:6px;">{r_emoji}</div>
                <div style="font-size:13px; color:{r_color}; font-weight:600; letter-spacing:1.5px; text-transform:uppercase; margin-bottom:4px;">
                    Predicted Heart Disease Risk
                </div>
                <div style="font-size:58px; font-weight:800; color:{r_color}; line-height:1.1; margin-bottom:10px;">
                    {risk:.1f}<span style="font-size:30px;">%</span>
                </div>
                <div style="background:{r_color}; color:white; display:inline-block;
                            padding:6px 22px; border-radius:20px; font-size:15px; font-weight:600; margin-bottom:16px;">
                    {r_label}
                </div>
                <div style="background:#e0e0e0; border-radius:8px; height:10px; width:100%; margin-top:4px;">
                    <div style="background:{r_color}; width:{min(risk,100):.0f}%; height:10px; border-radius:8px;
                                transition: width 0.5s ease;"></div>
                </div>
                <div style="font-size:12px; color:#888; margin-top:6px;">{risk:.1f}% out of 100%</div>
            </div>
            """, unsafe_allow_html=True)
            input_df = pd.DataFrame([input_data])
